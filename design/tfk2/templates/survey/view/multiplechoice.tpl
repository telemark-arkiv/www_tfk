...
{case match=1} {* Radio buttons in a row *}
  <div class="block float-break">
    <fieldset>
    {if $show_header}<legend>{if $show_question_number}{$question.question_number}. {/if}{$question.text|wash('xhtml')} {switch match=$question.num}{case match=1}<strong class="required">*</strong>{/case}{case match=2}<strong class="required">*</strong>{/case}{/switch}</legend>{/if}
...
{if $show_header}<label for="{$attr_id}"><strong>{if $show_question_number}{$question.question_number}. {/if}{$question.text|wash('xhtml')}</strong>{switch match=$question.num}{case match=1}<strong class="required">*</strong>{/case}{case match=2}<strong class="required">*</strong>{/case}{/switch}</label>{/if}
<div class="block float-break">
<select name="{$attr_name}" id="{$attr_id}" onclick="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );">
  {section var=option loop=$question.options}
