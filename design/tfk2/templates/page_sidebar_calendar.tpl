...
{def    $events=fetch( 'content', 'list', hash(
    ...
    $url_back=concat( $event_node.url_alias,  "/(month)/", sub($temp_month, 1), "/(year)/", $temp_year, "/", rand())
    $url_forward=concat( $event_node.url_alias, "/(month)/", sum($temp_month, 1), "/(year)/", $temp_year, "/", rand())
}

{if eq($temp_month, 1)}
    {set $url_back=concat( $event_node.url_alias,"/(month)/", "12", "/(year)/", sub($temp_year, 1), "/", rand())}
{elseif eq($temp_month, 12)}
    {set $url_forward=concat( $event_node.url_alias,"/(month)/", "1", "/(year)/", sum($temp_year, 1), "/", rand())}
{/if}
...
<table cellspacing="5" cellpadding="0" border="0" summary="Event Calendar">
<thead>
<tr class="calendar_heading">
    <th class="calendar_heading_prev first_col"><a href={$url_back|ezurl} title=" Forrige måned ">&#8249;&#8249;</a></th>
    <th class="calendar_heading_date" colspan="5">{$temp_ts|datetime( custom, '%F' )|upfirst()}&nbsp;{$temp_year}</th>
    <th class="calendar_heading_next last_col"><a href={$url_forward|ezurl} title=" Neste måned ">&#8250;&#8250;</a></th>
</tr>
...
{while le( $counter, $days )}
    ...
    {elseif and( eq($span1, 0 ), eq( $counter, 1 ) )}
        ...
        {/while}
    {/if}
    <td class="{if eq($counter, $temp_today)}ezagenda_selected{/if} {if and(eq($counter, $curr_today), eq($curr_month, $temp_month))}ezagenda_current{/if}{$css_col_class}" style="text-align: right;">
...
{def $now = currentdate()
           $children=fetch( 'content', 'list', hash( 'parent_node_id', $current_node_id,
                                                     'class_filter_type', 'include',
                                                     'class_filter_array', array( 'event' ),
...
<div class="content-view-children">
    {foreach $children as $child}
        {node_view_gui content_node=$child view=boxline}
    {/foreach}
  </div>
